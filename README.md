...
اسلیپ‌استریم-پلاس یک **توزیع‌کننده بار (Load Balancer)** و **پنل مدیریت پیشرفته** برای تانل‌های DNS مبتنی بر هسته قدرتمند [Slipstream-Rust](https://github.com/Mygod/slipstream-rust) است. این نرم‌افزار به صورت یکپارچه (Embedded) همراه با هسته کامپایل می‌شود و امکاناتی نظیر مدیریت اتصال، تعریف چند کاربر، پنل وب مدرن و مانیتورینگ مصرف پهنای باند را فراهم می‌کند.
